"""
Template: List Projects in a Team
Description: Retrieves and displays all projects in a specified team
Prerequisites: Valid API credentials, Team ID
API Endpoint: GET /api/v1/teams/{team_id}/projects
"""
...
response = api_request('GET', f'/api/v1/teams/{TEAM_ID}/projects')

# Display results
print(f"\n{'='*70}")
print(f"Projects in Team {TEAM_ID}")
print(f"Found {len(response['data'])} project(s)")
print(f"{'='*70}\n")
...
for project in response['data']:
    ...
    visibility = attrs.get('visibility', 'N/A')

    status = "🗄️  [ARCHIVED]" if archived else "✓ [ACTIVE]"

    print(f"{status} Project ID: {project_id}")
    print(f"     Name: {name}")
    print(f"     Visibility: {visibility}")
    print()

print(f"{'='*70}")
print(f"\n💡 Tip: Use a Project ID in other templates to work with experiments and tasks.\n")
